...
public interface BillRepository extends JpaRepository<Bill, Integer> {
    ...
    @Query("SELECT MONTH(b.date) AS month, SUM(DISTINCT b.totalAmount) AS revenue, COUNT(DISTINCT ol.cart.customer) AS customerCount " +
            "FROM Bill b " +
            "JOIN OrderList ol " +
            "ON b.order = ol.order " +
    ...
    @Query(value = "SELECT DATEPART(WEEKDAY, b.Date) AS dayOfWeek, SUM(b.Total_Amount) AS revenue " +
            "FROM Bill b " +
            "WHERE CAST(b.Date AS DATE) BETWEEN :startDate AND :endDate " +
            "GROUP BY DATEPART(WEEKDAY, b.Date) ORDER BY dayOfWeek", nativeQuery = true)
